Shader "gRally/grs_DNS_VM"
{
	Properties
	{
		...
		[Enum(UV0,0,UV1,1,UV2,2,UV3,3)] _MainBlendID ("Albedo Blend UV", Float) = 0
		_BumpMapBlend ("Normal Blend", 2D) = "bump" {}
		[Enum(UV0,0,UV1,1,UV2,2,UV3,3)] _BumpBlendID ("Normal Blend UV", Float) = 0
		_SpecMapBlend ("Specular Blend", 2D) = "gray" {}
		[Enum(UV0,0,UV1,1,UV2,2,UV3,3)] _SpecBlendID ("Specular Blend UV", Float) = 0		
	}
	...
	SubShader
	{
		...
		void vert (inout myAppdata v, out Input o)
		{
            ...
            // spec2
            o.spec2 = v.texcoord.xy;
            if(_SpecBlendID == 1) 		o.spec2 = v.texcoord1.xy;
            else if(_SpecBlendID == 2)	o.spec2 = v.texcoord2.xy;
            else if(_SpecBlendID == 3)	o.spec2 = v.texcoord3.xy;
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			// specular intensity
			fixed4 sW = tex2D(_SpecTex, IN.spec1) * IN.vColor.r;
			fixed4 sB = tex2D(_SpecMapBlend, IN.spec2) * (1 - IN.vColor.r);
			fixed4 s = (sW + sB) * _SpecColor;
			o.Gloss = s.r;
